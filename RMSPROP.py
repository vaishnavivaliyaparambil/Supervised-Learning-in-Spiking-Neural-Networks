...
for hhh in sim:
    ...
    #     # inputWeightsmod = inputWeights
        ...
        filename3 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testnrefw.pt".format(s)
...
#         filename2 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}TestninputST.pt".format(s)
#         interrefSpikeTrain = next(iter(torch.load(filename2)))
#         filename4 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}TestnrefST.pt".format(s)
...
#             filename1 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair20Testnweights6.pt"
#             ran_num2 = torch.load(filename1)[-1].T
            
            ran_num2 = (10.0 - 15.0) * torch.rand((num_synapses_in, 1), device = mydevice) + 15.0
            ...
            filename1 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testinitweights1.pt".format(s)
...
#                         
                        ssd += up_w**2
                        rms = (rms*rho)+((ssd)*(1-rho))
                        denom = torch.sqrt(rms)
                        lr = mu+torch.zeros(15)
                        if denom.any() != 0:
                            idx = torch.where(denom!=0)
                            
                            for n in range(len(idx[0])):
                                lr[idx[0][n]] =  mu/denom[idx[0][n]]
                            
#                             print('lr',torch.where(lr!=0))
                        else: 
                            lr = mu
                        update = (lr*up_w.T)
                        update_mat = torch.zeros(1,15,1000)
                        for r in range(1000):
                            update_mat[:,:,r] = update
#                         print('update_mat',update_mat)
                        update_tens_cap = torch.clamp(update_mat ,min = -cap, max=cap)
...
#                         filename100 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testnweights_updates.pt".format(s)
#                         torch.save(synaptic_wt_list,filename100)          
#                 filename1 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testnweights.pt".format(s)
#                 weightup_dict = {}
#                 weightupdate_list.append(synaptic_wt_list[-1])
#                 weightup_dict[w] = weightupdate_list
#                 torch.save(weightupdate_list,filename1)
#                 filename2 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testnerror.pt".format(s)
#                 error_dict = {}
...
#         filename2 = "/home/vaishnavi/multiple_opt/rmsprop/multiple/nPair{}Testnmu.pt".format(s)
...
import torch as torch
filename = "/home/vaishnavi/multiple_opt/rmsprop/nPair20Testnweights.pt"
